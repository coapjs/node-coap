...
CoAPServer.prototype._handle = function(packet, rsinfo) {
  ...
  var sock      = this._sock
    , acks      = this._acks
    , cached    = cache.peek(toKey(rsinfo.address, rsinfo.port, packet, true))
    , Message   = OutMessage
    , that = this
    , request
    , response
  ...
  if (cached && !packet.ack && !packet.reset)
    ...
    if (cached.response && packet.reset)
      cached.response.end()
    return cache.del(toKey(rsinfo.address, rsinfo.port, packet, false))
  }
  ...
  response = new Message(packet, function(response, packet) {
    ...
    cache.set(toKey(rsinfo.address, rsinfo.port,
  ...
  var block2Option = getOption(request.options, 'Block2')
  if (packet.token && packet.token.length > 0 && block2Option && block2Option.num && block2Option.blockNr > 0) {
    var value = cache.get(packet.token);
    if (value) {
      console.log('YEAH CACHED!');
      response.end(value);
