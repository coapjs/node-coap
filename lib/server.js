...
/**
 * @typedef { import('../index').CoapServerOptions } CoapServerOptions
 * @typedef { import('../index').requestListener } requestListener
...
class CoAPServer extends events.EventEmitter {
    /**
     *
     * @param {CoapServerOptions | requestListener} [serverOptions]
    ...
    constructor (serverOptions, listener) {
        ...
        // Multicast settings
        this._multicastAddress = this._options.multicastAddress
            ? this._options.multicastAddress
            : null
        this._multicastInterface = this._options.multicastInterface
     ...
     * @param {CoapPacket} [packet]
     * @param {string} [code="5.00"]
     */
     ...
     * @param {(error: Error | null, bytes: number) => void} callback
     */
    ...
    /**
     *
     * @param {number | events.EventEmitter | ((err?: Error) => void)} [portOrCallback]
     * @param {string | ((err?: Error) => void)} [addressOrCallback]
     * @param {(err?: Error) => void} [done]
     * @returns {this}
     */
    listen (portOrCallback, addressOrCallback, done) {
        /** @type {number | events.EventEmitter} */
        let port = null
        if (portOrCallback == null) {
            port = parameters.coapPort
        } else if (typeof portOrCallback === 'function') {
            done = portOrCallback
            port = parameters.coapPort
        } else {
            port = portOrCallback
        ...
        if (this._options.reuseAddr == null) {
            this._options.reuseAddr = true
        }

        if (portOrCallback instanceof events.EventEmitter) {
            /** @type {any} */
     ...
     * @param {CoapPacket} packet The packet that was sent from the client.
     * @param {AddressInfo} rsinfo Connection info
     */
    _handle (packet, rsinfo) {
        ...
        const lru = this._lru
        /**
         * FIXME: Workaround to prevent type errors
         *
         * @type {any}
         * */
        ...
        if (packet.token && packet.token.length > 0) {
            ...
            if (block2Buff != null && block2Buff instanceof Buffer) {
        ...
        // else goes here?
        {
            ...
            if (block1Buff != null) {
                ...
                if (block1Buff instanceof Buffer) {
                    blockState = parseBlockOption(block1Buff)
                }
    ...
    _toCacheKey (request, packet) {
        if (packet.token && packet.token.length > 0) {
            return `${packet.token.toString('hex')}/${this._options.clientIdentifier(
                request
            )}`
        }
     ...
     * @param {IncomingMessage} request
     * @param {CoapPacket} packet
     * @param {boolean} appendToken
     * @returns {string}
     */
    _toKey (request, packet, appendToken) {
        let result = `${this._options.clientIdentifier(request)}/${packet.messageId}`
