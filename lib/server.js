...
CoAPServer.prototype.listen = function (port, address, done) {
    ...
    if (this._sock) {
        if (done) { done(new Error('Already listening')) } else { throw new Error('Already listening') }
    ...
    if (port instanceof events.EventEmitter) {
        ...
        this._sock.bind(port, address || null, function () {
            try {
                ...
                if (done) { return done(err) } else { throw err }
            }
...
CoAPServer.prototype._handle = function (packet, rsinfo) {
  ...
  const generateResponse = function () {
    ...
    const cacheKey = toCacheKey(rsinfo.address, rsinfo.port, packet)
    ...
    const generateResponse = function () {
        const response = new Message(packet, function (response, packet) {
            ...
            const key = toKey(rsinfo.address, rsinfo.port,
                packet, packet.ack || !packet.confirmable)
            lru.set(key, buf)
            buf.sender = sender
    ...
    // else goes here?
    {
        ...
        if (block1Buff) {
            ...
            if (blockState) {
                ...
                const incomingByteIndex = blockState.sequenceNumber * blockByteSize
...
let maxBlock2 = Math.pow(2, Math.floor(Math.log(parameters.maxPacketSize) / Math.log(2)))
if (maxBlock2 > Math.pow(2, (6 + 4))) { maxBlock2 = Math.pow(2, (6 + 4)) }
...
OutMessage.prototype.end = function (payload) {
    ...
    if (!payload ||
    ((!requestedBlockOption) && (payload.length < parameters.maxPacketSize))) { return OutgoingMessage.prototype.end.call(this, payload) }
