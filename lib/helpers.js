...
module.exports.parseBlock2 = function parseBlock2(options) {
  for (var i in options) {
    if (options[i].name == 'Block2') {
      ...
      switch (block2Value.length) {
        ...
        break
        default:
        throw new Error('Block2 is more than 3 bytes: '+block2Value.length)
      }
      // limit value of size is 1024 (2**(6+4))
      if (block2Value.slice(-1)[0] == 7) {
        throw new Error('Block size is bigger than 1024')
      }
      return {
...
module.exports.createBlock2 = function createBlock2(requestedBlock) {
  ...
  if (num <= 0xf) { 
    ...
    throw new Error('too big block2 number')
  }
