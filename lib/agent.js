...
Agent.prototype._handle = function handle(msg, rsinfo) {
  ...
  if (packet.block2) {
    ...
    console.log('block2 packet number', packet.block2.num)
    // console.log('block2 packet size', packet.block2.size)
    if (packet.block2.moreBlock2) {
      // console.log('more block2 packet is waiting')

      // delete this._msgIdToReq[packet.messageId]
      var originalRequest = req._packet
      delete this._msgIdToReq[originalRequest.messageId]
      originalRequest.messageId = that._nextMessageId()
      // console.log(originalRequest)
      var nextRequest = packetSetBlock(originalRequest, packet.block2.num, packet.block2.size)
      nextRequest = packetRemoveObserve(nextRequest)
      ...
      req.sender.send(buf)
      // req.sender = new RetrySend(that._sock, url.port, url.hostname || url.host)
      // console.log('--------nex request')
      // console.log(nextRequest)
      return
    }
    else {
  ...
  if (req.url.observe && packet.code !== '4.04') {
    console.log('begin new notification')
...
function packetSetBlock(packet, number, size) {
  if (size < 16) size = 16
...
function packetRemoveObserve(packet) {
  for (i in packet.options) {
    if (packet.options[i].name == 'Observe') {
      packet.options.splice(i, 1)
    }
  }
  return packet
}
