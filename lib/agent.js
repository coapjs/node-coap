...
var IncomingMessage = require('./incoming_message')
var OutgoingMessage = require('./outgoing_message')
var ObserveStream = require('./observe_read_stream')
var RetrySend = require('./retry_send')
var parseBlock2 = require('./helpers').parseBlock2
var createBlock2 = require('./helpers').createBlock2
var getOption = require('./helpers').getOption
var maxToken = Math.pow(2, 32)
var maxMessageId = Math.pow(2, 16)
var pf = require('./polyfill')
...
Agent.prototype._handle = function handle (msg, rsinfo, outSocket) {
  ...
  } else if (packet.code !== '0.00' &&
    (req._packet.token.length !== packet.token.length || pf.compareBuffers(req._packet.token, packet.token) !== 0)) {
...
Agent.prototype.request = function request (url) {
  ...
  var req
  var options = url.options || url.headers
  var option
  var that = this
  var multicastTimeout = url.multicastTimeout !== undefined ? parseInt(url.multicastTimeout, 10) : 20000
