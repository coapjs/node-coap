...
const isOption = require('./helpers').isOption
...
function handleProxyResponse (request, next) {
    ...
    const originalProxiedRequest = request.server._proxiedRequests[request.packet.token.toString('hex')]
    if (originalProxiedRequest) {
        request.server._sendReverseProxied(request.packet, originalProxiedRequest.rsinfo)

        if (!isObserve(request.packet)) { delete request.server._proxiedRequests[request.packet.token.toString('hex')] }
