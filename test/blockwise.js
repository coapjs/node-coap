...
var coap        = require('../')
  ...
  , parseBlock1 = require('../lib/helpers').parseBlock1
  , dgram       = require('dgram')
...
describe.only('blockwise1', () =>{
  ...
  it('should receive full response payload', function(done) {
    ...
    .setOption('Block1', Buffer.of(0x0)) // early negotation with block size = 16, almost 10000/16 = 63 blocks
    .on('response', function(res) {
      expect(res.payload).to.eql(payload)
      //expect(cache.get(res._packet.token.toString())).to.not.be.undefined
      setImmediate(done)
  ...
  function sendNextBlock1(req_token, req_block1_num) {
    var packet = {
        messageId: 1100 + req_block1_num
      , token: req_token
      , options: [{
            name: 'Block1'
          , value: Buffer.of(req_block1_num << 4)
        }]
    }
    send(generate(packet))
  }

  function parallelBlock1Test(done, checkNReq, checkBlock1Message, checkNormalReq) {
    var payload_len = 32+16+1
    var payload_req1 = Buffer.alloc(payload_len)
    var payload_req2 = Buffer.alloc(payload_len)
    var req1_token = Buffer.alloc(4)
    ...
    // Send first request, initiate blockwise transfer from server
    sendNextBlock1(req1_token, req1_block1_num)

    client.on('message', function(msg, rinfo) {
      checkBlock1Message(msg, payload_req1, req1_block1_num, payload_len)

      var expectMore = (req1_block1_num + 1) * 16 <= payload_len
      if (expectMore) {
        // Request next block after 50 msec delay
        req1_block1_num++
    ...
    req_client2.setOption('Block1', Buffer.of(0x10)) // request from block 1, with size = 16
