...
// Create the server as a proxy in order to have the same source and destination ports for the bidirectional communication due to firewall issues.
var proxy = coap.createServer({ proxy: true }, function(request, response) {
    ...
    if (parsedURL.pathname == '/initiate') {    
      ...
      // Send ACK
      response.end('Proxy received initiate');  
